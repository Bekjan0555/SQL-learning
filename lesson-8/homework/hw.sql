...
FROM Products;
--15.
-- Create the new table to store pivoted data
...
-- Insert pivoted data into the new table
...
CREATE TABLE Population_Each_City (
    ...
    Mirobod DECIMAL(10,2),
    Yashnobod DECIMAL(10,2)
);
...
--22.
-- Transform Population_Each_City back to original City_Population format using UNPIVOT
SELECT 
    ROW_NUMBER() OVER (ORDER BY year, district_name) AS district_id,
    district_name,
    population,
    year
FROM (
